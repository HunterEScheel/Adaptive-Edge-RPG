...
import { Consumable, Equipment, Weapon } from "@/constants/Item";
...
const inventorySlice = createSlice({
  ...
  reducers: {
    ...
    toggleAttunementEquipment: (state, action: PayloadAction<string>) => {
      const equipmentId = action.payload;
      const newEquipment = state.equipment.map((item: any) => 
        (item.id === equipmentId ? { ...item, attune: !item.attune } : item)
      );
    ...
    addConsumable: (state, action: PayloadAction<Omit<Consumable, "id">>) => {
      ...
      const existingIndex = state.consumables.findIndex(
