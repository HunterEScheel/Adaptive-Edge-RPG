...
export function WeaponSkillManager() {
    ...
    const renderSkillItem = ({ item }: { item: WeaponSkill }) => {
        return (
            <View style={[cssStyle.sectionContainer, cssStyle.row, { justifyContent: "space-between", paddingVertical: 0 }]}>
                <ThemedText style={cssStyle.defaultBold}>
                    {item.weaponHeft}
                    {item.weaponHeft === "Unarmed" ? "" : "- " + item.weaponType}
                </ThemedText>
                                ...
                                <FlatList
                                    ...
                                    renderItem={(weapon) => (
                                        <TouchableOpacity key={weapon.index} style={cssStyle.suggestionItem} onPress={() => selectWeaponSkill(weapon.item)}>
                                            <ThemedText>
                                                {weapon.item.weaponHeft}
                                                {weapon.item.weaponHeft === "Unarmed" ? "" : " - " + weapon.item.weaponType}
                                            </ThemedText>
                                        </TouchableOpacity>
                                    )}
                                />
