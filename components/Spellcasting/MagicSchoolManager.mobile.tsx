...
export function MagicSchoolManagerMobile() {
  ...
  const handleSchoolLevelChange = (schoolName: string, addLevels: 1 | -1) => {
    ...
    if (!school) return;
    const newCost = addLevels > 0 ? MAGIC_SCHOOL_COST : -MAGIC_SCHOOL_COST;
    const newBuildPoints = base.buildPointsRemaining - newCost;
    if (newBuildPoints < 0) {
      Alert.alert("Not Enough Build Points", `You need ${newCost - base.buildPointsRemaining} more build points to increase ${schoolName} to level ${school.levels + 1}.`);
      return;
    }
    dispatch(
  ...
  const handleLearnSchool = () => {
    ...
    if (magic.magicSchoolCredit) {
      ...
      Alert.alert("Not Enough Build Points", `You need ${MAGIC_SCHOOL_COST} build points to learn ${selectedSchool.name}. You currently have ${base.buildPointsRemaining}.`);
    }
  };
  ...
  const handleRemoveSchool = (schoolName: string) => {
    ...
    if (spellsFromSchool > 0) {
      Alert.alert("Cannot Remove School", `You have ${spellsFromSchool} spell${spellsFromSchool !== 1 ? "s" : ""} from ${schoolName}. Remove all spells from this school first.`);
      return;
    }
    ...
    if (known) {
        ...
        >
          ...
          <View style={{}}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <ThemedText style={{ fontSize: 10, color: "#A00", marginLeft: 6 }}>{base.buildPointsRemaining < nextLevelCost && "Next Level: " + nextLevelCost + " BP"}</ThemedText>
            </View>
      ...
      return (
        ...
        >
          ...
          >
            <ThemedText style={{ fontSize: 10, color: "white", fontWeight: "600" }}>{magic.magicSchoolCredit ? "FREE" : `${MAGIC_SCHOOL_COST} BP`}</ThemedText>
          </View>
        </Pressable>
      );
    }
        ...
        <View>
          <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 6 }}>
            ...
            {base.buildPointsRemaining < MAGIC_SCHOOL_COST && !magic.magicSchoolCredit && (
              <ThemedText style={{ fontSize: 10, color: "#e74c3c", marginLeft: 6 }}>(Need {MAGIC_SCHOOL_COST - base.buildPointsRemaining} BP)</ThemedText>
            )}
          </View>
          {MAGIC_SCHOOLS.filter((school) => !isSchoolKnown(school.name)).map(renderSchoolCard)}
        </View>
          ...
          <Pressable style={[cssStyle.modalView, { padding: 16 }]} onPress={(e) => e.stopPropagation()}>
            {selectedSchool && (
              <>
                ...
                <ThemedText style={{ fontSize: 12, color: "#666", textAlign: "center", marginVertical: 8 }}>{selectedSchool.description}</ThemedText>
                ...
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <Pressable style={[cssStyle.defaultButton, cssStyle.secondaryColors, { paddingVertical: 3 }]} onPress={() => setModalVisible(false)}>
                    <ThemedText style={{ fontSize: 12, color: "#666" }}>Cancel</ThemedText>
                  </Pressable>
                  <Pressable style={[cssStyle.defaultButton, { backgroundColor: selectedSchool.color, paddingVertical: 3 }]} onPress={handleLearnSchool}>
