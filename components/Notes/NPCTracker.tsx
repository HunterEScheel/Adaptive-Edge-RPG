...
export const NPCTracker: React.FC = () => {
  ...
  const renderRelationshipIndicator = (level: RelationshipLevel, onUpdate?: (newLevel: RelationshipLevel) => void) => {
    ...
    return (
      <View style={cssStyle.relationshipContainer}>
        <ThemedText style={cssStyle.relationshipLabel}>{getRelationshipDescription(level)}</ThemedText>
  ...
  const renderNPC = ({ item }: { item: NPC }) => {
    ...
    return (
      <ThemedView style={cssStyle.npcCard}>
        <View style={cssStyle.npcHeader}>
          <ThemedText style={cssStyle.npcName}>{item.name}</ThemedText>
          <Pressable style={cssStyle.expandButton} onPress={() => toggleExpanded(item.id)}>
            <MaterialIcons name={isExpanded ? "expand-less" : "expand-more"} size={24} color="#444" />
          </Pressable>
        </View>

        <ThemedText style={cssStyle.npcDescription}>{item.description}</ThemedText>

        <ThemedText style={cssStyle.relationshipText}>{getRelationshipDescription(item.relationshipLevel)}</ThemedText>

        {isExpanded && (
          <>
            {item.notes ? (
              <View style={cssStyle.notesContainer}>
                <ThemedText style={cssStyle.notesHeader}>Notes:</ThemedText>
                <ThemedText style={cssStyle.notesContent}>{item.notes}</ThemedText>
              </View>
            ) : null}

            <View style={cssStyle.actionButtons}>
              <Pressable style={cssStyle.iconButton} onPress={() => openEditModal(item)}>
                <MaterialIcons name="edit" size={20} color="#444" />
              </Pressable>
              <Pressable style={cssStyle.iconButton} onPress={() => handleDeleteNPC(item.id)}>
  ...
  return (
    <ThemedView style={cssStyle.container}>
      <Collapsible title="NPC Relationships">
        <ThemedView style={cssStyle.npcContainer}>
          {npcs.length === 0 ? (
            ...
            <FlatList
              ...
              contentContainerStyle={cssStyle.npcList}
          ...
          <Pressable style={cssStyle.addButton} onPress={() => setModalVisible(true)}>
            ...
            <ThemedText style={cssStyle.addButtonText}>Add NPC</ThemedText>
          </Pressable>
          ...
          <Modal visible={modalVisible} transparent={true} animationType="slide" onRequestClose={() => resetForm()}>
            <View style={cssStyle.modalOverlay}>
              <ThemedView style={cssStyle.modalContent}>
                <ThemedText style={cssStyle.modalHeader}>{selectedNPC ? "Edit NPC" : "Add New NPC"}</ThemedText>
                ...
                <ThemedText style={cssStyle.relationshipHelp}>
                ...
                <View style={cssStyle.modalButtons}>
                  <Pressable style={[cssStyle.modalButton, cssStyle.cancelButton]} onPress={resetForm}>
                    <ThemedText style={cssStyle.buttonText}>Cancel</ThemedText>
                  </Pressable>
                  <Pressable style={[cssStyle.modalButton, cssStyle.saveButton]} onPress={selectedNPC ? handleUpdateNPC : handleAddNPC}>
