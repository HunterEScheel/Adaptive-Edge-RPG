...
import { FlatList, Modal, TouchableOpacity, View, TextInput } from "react-native";
...
const CharacterPresetManager = () => {
  ...
  const renderPresetItem = ({ item }: { item: Character }) => (
    <View style={cssStyle.card}>
      ...
      <TouchableOpacity style={[cssStyle.compactButton, cssStyle.dangerButton]} onPress={() => handleDeletePreset(item)}>
  ...
  return (
    <ThemedView style={cssStyle.container}>
      <View style={cssStyle.headerRow}>
        ...
        <TouchableOpacity style={cssStyle.actionButton} onPress={() => setShowSaveModal(true)}>
          <ThemedText style={cssStyle.buttonText}>Save Current Character</ThemedText>
        </TouchableOpacity>
      </View>

      {presets.length > 0 ? (
        <FlatList data={presets} renderItem={renderPresetItem} keyExtractor={(item) => item.base.id.toString()} contentContainerStyle={{ paddingBottom: 16 }} />
      ) : (
        ...
        <View style={cssStyle.modalOverlay}>
          <ThemedView style={cssStyle.modalView}>
            ...
            <View style={cssStyle.modalButtons}>
              ...
              <TouchableOpacity style={cssStyle.actionButton} onPress={handleSavePreset}>
            ...
            {selectedPreset && (
              <>
                ...
                <View style={cssStyle.modalButtons}>
                  ...
                  <TouchableOpacity style={cssStyle.actionButton} onPress={() => handleLoadPreset(selectedPreset)}>
