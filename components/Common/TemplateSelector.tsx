...
export const TemplateSelector: React.FC<TemplateSelectorProps> = ({ onSelectTemplate, onBack }) => {
  ...
  return (
    <ThemedView style={(cssStyle.container, { height: "60%" })}>
      <ThemedText style={[cssStyle.headerText]}>Select a Character Template</ThemedText>

      {presets.length === 0 ? (
        <View style={{ padding: 20 }}>
          <ThemedText style={{ textAlign: "center", marginBottom: 20 }}>No templates available. Create a character from scratch or ask your GM for template files.</ThemedText>
        </View>
      ) : (
        <FlatList
          data={presets}
