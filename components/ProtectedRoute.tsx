...
export function ProtectedRoute({ children }: ProtectedRouteProps) {
    ...
    if (!isCharacterLoaded) {
        return Platform.OS === "web" ? null : <View style={{ backgroundColor: "#fff" }} />;
    }
    ...
    const wrappedChildren = React.Children.map(children, (child: React.ReactNode, index: number) => {
        ...
        if (Array.isArray(child)) {
            return child.map((c: React.ReactNode, i: number) =>
                typeof c === "string" || typeof c === "number" ? <ThemedText key={`text-${index}-${i}`}>{String(c)}</ThemedText> : c
            );
        }
    ...
    return Platform.OS === "web" ? <>{wrappedChildren}</> : <View style={{}}>{wrappedChildren}</View>;
}
