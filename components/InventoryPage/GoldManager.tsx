  ...
  return (
    <ThemedView style={cssStyle.container}>
      <View style={[cssStyle.row, cssStyle.card, { backgroundColor: "rgba(255, 215, 0, 0.1)" }]}>
        ...
        <Pressable style={cssStyle.compactButton} onPress={() => setShowAdjustment(!showAdjustment)} accessibilityLabel={showAdjustment ? "Hide gold adjustment panel" : "Show gold adjustment panel"}>
      ...
      {showAdjustment ? (
        <ThemedView style={cssStyle.card}>
          ...
          <View style={cssStyle.modalButtons}>
            <Pressable style={cssStyle.successButton} onPress={() => handleAdjustGold(true)} accessibilityLabel={`Add ${adjustmentAmount || "amount"} gold`}>
              <ThemedText style={cssStyle.buttonText}>Add</ThemedText>
            </Pressable>

            <Pressable style={cssStyle.dangerButton} onPress={() => handleAdjustGold(false)} accessibilityLabel={`Subtract ${adjustmentAmount || "amount"} gold`}>
          ...
          <View style={cssStyle.adjustmentRow}>
            {[1, 10, 25, 100].map((amount) => (
              <Pressable
                key={amount}
                style={[cssStyle.compactButton, { backgroundColor: "rgba(255, 215, 0, 0.2)" }]}
                onPress={() => {
