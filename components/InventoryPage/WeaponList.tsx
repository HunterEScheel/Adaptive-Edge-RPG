...
export function WeaponList({ variant = "full" }: WeaponListProps) {
      ...
      <View style={cssStyle.itemContainer}>
        <View style={cssStyle.itemHeader}>
          ...
          <View style={[cssStyle.row, { gap: 8 }]}>
            <Pressable style={[cssStyle.defaultButton, item.equipped ? cssStyle.secondaryColors : cssStyle.primaryColors]} onPress={() => handleEquipToggle(item.id)}>
  ...
  if (variant === "mini") {
    return (
      <>
        <ListManagerMobile<Weapon>
          title="Weapons"
          data={weapons}
          renderItem={renderMiniWeaponItem}
