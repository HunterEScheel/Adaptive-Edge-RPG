      ...
      <Modal animationType="slide" transparent={true} visible={showAddFlawModal} onRequestClose={() => setShowAddFlawModal(false)}>
        <View style={cssStyle.modalOverlay}>
          <ThemedView style={cssStyle.modalView}>
            ...
            <TextInput style={cssStyle.input} placeholder="Flaw name" value={newFlawName} onChangeText={setNewFlawName} />

            <TextInput style={[cssStyle.input, { height: 80 }]} placeholder="Description" value={newFlawDescription} onChangeText={setNewFlawDescription} multiline />

            <ThemedText style={cssStyle.label}>Severity:</ThemedText>
            <View style={cssStyle.row}>
              {(["quirk", "flaw", "vice"] as FlawSeverity[]).map((severity) => (
                <Pressable key={severity} style={[{ marginRight: 10 }, newFlawSeverity === severity ? cssStyle.primaryButton : cssStyle.secondaryButton]} onPress={() => setNewFlawSeverity(severity)}>
                  <ThemedText style={cssStyle.buttonText}>{severity}</ThemedText>
                </Pressable>
              ))}
            </View>
            ...
            <View style={cssStyle.headerRow}>
              ...
              <Pressable style={[cssStyle.primaryButton, { width: "45%" }]} onPress={handleAddFlaw} disabled={!newFlawName.trim() || !newFlawDescription.trim()}>
